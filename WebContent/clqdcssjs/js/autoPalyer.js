...
self.go(goTo);},getIndex:function(){var self=this;return self.index||0;},getName:function(){var self=this;return self.data[self.index||0].name||0;},getBh:function(){var self=this;return self.data[self.index||0].bh||0;},getFileUrl:function(){var self=this;return self.data[self.index||0].fileUrl||0;},getClxh:function(){var self=this;return self.data[self.index||0].clxh||0;},getBdxx:function(){var self=this;return self.data[self.index||0].bdxx||0;},getPicTitle:function(){var self=this;return self.data[self.index||0].picTitle||0;},getisClick:function(){var self=this;return self.data[self.index||0].isClick||0;},getCurid:function(){var self=this;return self.data[self.index||0].curid||0;},getTotal:function(){var self=this;return self.data.length||0;},_isDefined:function(variable){return typeof(variable)!="undefined"},_eventBinding:function(){var self=this;self._bindingButClick(self.butPlay,function(){self.play();},true);self._bindingButClick(self.butStop,function(){self.stop();},true);self._bindingButClick(self.butPause,function(){self.pause();},true);if(null!=self.butSpeedGroup){self.butSpeedGroup.find("a").click(function(){self.butSpeedGroup.find("a").removeClass("focus")
self.setSpeed(Number(jQuery(this).attr("speed")));jQuery(this).addClass("focus");});}
self._bindingButClick(self.butPrev,function(){self.prev();changePlanBddxx();},true);self._bindingButClick(self.butNext,function(){self.next();changePlanBddxx();},true);self._bindingButClick(self.butPrevGroup,function(){self.prevGroup();changePlanBddxx();},true);self._bindingButClick(self.butNextGroup,function(){self.nextGroup();changePlanBddxx();},true);if(null!=self.butExamine){self.butExamine.hover(function(){if(self.playFlag){self.pauseFlag=true;}},function(){if(self.playFlag){self.pauseFlag=false;}});}
...
self.nav.html(html);self.nav.find("a").click(function(){self.go(Number(jQuery(this).attr("idx")));changePlanBddxx();});},_showNumberRefresh:function(){var self=this;if(null!=self.showTotal){self.showTotal.html(self.getTotal());}
...
but.click(function(){func.call();});}},_catchKeyEvent:function(){var self=this;jQuery("body").keydown(function(event){event=event||window.event;var url=null;switch(event.keyCode){case 37:self.prev();changePlanBddxx();break;case 39:self.next();changePlanBddxx();break;}
