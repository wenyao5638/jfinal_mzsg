 ...
 $(function(){
	 ...
	 var contextPath = $('#contextPath').val();
			...
			$('#dg').datagrid({
				...
				loadMsg : '数据装载中......', 
				fit: true,
				url:'<%=contextPath%>/sgdcsp/sgdcspList.do?zt=<s:property value="zt"/>&cz='+cz,
				frozenColumns:[[
...
<%--	window.parent.addTab("社工调动申请清单","<%=contextPath%>/sgdcsq/addnew.do");--%>
<%--}--%>
//查看
function chakan(id){
	window.parent.closeTab("社工调动申请清单");
	window.parent.addTab("社工调动申请清单","<%=contextPath%>/sgdcsp/chakan.do?id="+id);
}
//审批
function sqsp(id,ryid){
	window.parent.closeTab("社工调动申请清单");
	window.parent.addTab("社工调动申请清单","<%=contextPath%>/sgdcsp/sqsp.do?id="+id+"&ryid="+ryid);
}

function edit(id,bz){
	window.parent.closeTab("社工调动申请清单");
	window.parent.addTab("社工调动申请清单","<%=contextPath%>/sgdcsp/sqsp.do?pid=${pid}&id="+id+"&cz="+cz+"&bz="+bz);
}
