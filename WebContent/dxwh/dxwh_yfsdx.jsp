...
<title>再生育管理系统>>短信维护>>已发送短信</title>
 ...
 $(function(){
	 var contextPath = $('#contextPath').val();
			$('#tt2').datagrid({
				...
				loadMsg : '数据装载中......', 
				fit: true,
				url:'<%=contextPath%>/dxwh/yfsdxList.do?qx=<s:property value="qx"/>',
				idField:'area',
				columns:[[
				    ...
				    {field:'sfssj',title:'发送时间',width:120}
					]],
				toolbar:[{
					text:'刷新',
					iconCls:'icon-reload',
...
function search(){
	...
	var sqr = $('#sqr').val();
	$('#tt2').datagrid({
		queryParams: {"search_yfsdx.jsdx":jsdx,"search_yfsdx.fsnr":fsnr,"sqr":sqr}
	});
	displayMsg();  
}
 </script>
...
<div data-options="region:'center',title:'已发送短信记录'" style="background:#fafafa; padding:0px;overflow:hidden">
